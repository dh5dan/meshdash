<script>
    $(function ($) {
        ...
        function dialogOpenStreet(contentHtml, title, width, dateFrom, dateTo, nodeMapJsonFileUpdateInfo, mode = 'dialog')
        {
            ...
            let dialogOpenHandler = function () {
                ...
                $.getJSON('nodemap.json?_=' + Date.now()).done(function (data) {
                    ...
                    let from = new Date(dateFrom).toLocaleDateString('de-DE');
                    let to   = new Date(dateTo).toLocaleDateString('de-DE');

                    $('#dateRangeInfo').text(`Zeitraum: ${from} – ${to}`);

                    //Datumsbereich
                    L.control.custom({
                        position: 'bottomleft',
                        content: '<div class="leaflet-control-date">' + from + ' – ' + to + ' | Stand: ' + nodeMapJsonFileUpdateInfo + '</div>',
                        classes: 'leaflet-bar',
                        style: {
