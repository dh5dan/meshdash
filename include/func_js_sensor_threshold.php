<script>
    $(function ($) {
        ...
        $("#btnSaveSensorThresholdTop, #btnSaveSensorThresholdBottom").on("click", function ()
        {
            ...
            let sensorThTempIntervallSec = $("#sensorThTempIntervallSec").val();
            ...
            // Prüfen, ob die Eingabe KEINE ganze Zahl >= 60 ist
            if (!/^\d+$/.test(sensorThTempIntervallSec) || parseInt(sensorThTempIntervallSec, 10) < 60) {
                width       = 500;
                outputMsg = 'Der Intervallwert muss eine ganze Zahl >= 60 sein.';
                outputMsg += '<br><br>Bitte prüfen.';
            ...
            if ($("#sensorThIna226vBusEnabled").length)
            {
                ...
                let sensorThIna226IntervallSec = $("#sensorThIna226IntervallSec").val().trim();

                // Prüfen, ob die Eingabe KEINE ganze Zahl >= 60 ist
                if (!/^\d+$/.test(sensorThIna226IntervallSec) || parseInt(sensorThIna226IntervallSec, 10) < 60) {
                    width       = 500;
                    outputMsg = 'Der INA226-Intervallwert muss eine ganze Zahl >= 60 sein.';
                    outputMsg += '<br><br>Bitte prüfen.';
