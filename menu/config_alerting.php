...
if ($sendData === '2')
{
    ...
    {
        echo '<br><span class="successHint">Eintrag: ' . $deleteNotifyItemId . ' erfolgreich gelöscht.</span>';
    }
    else
    {
        echo '<br><span class="failureHint">Fehler beim Löschen von Eintrag: ' . $deleteNotifyItemId . '</span>';
    }
}
...
if (count($arrayNotificationData) > 0)
{
    ...
    echo '<td>Snd-File:</td>';
    echo '<td><select name="notifySoundFile[' . $notifyId . ']" id="notifySoundFile_' . $notifyId . '" disabled>';
    selectSoundFile(showAlertMediaFiles(false), $notifySoundFile);
    echo '</td>';
    echo '<td></td>';
    echo '</tr>';
